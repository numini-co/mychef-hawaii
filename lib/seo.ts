...
export function sitemapLocs(hostname: string): { loc: string; changefreq: string; priority: string }[] {
  ...
  const extras = (fromHost ? [fromHost] : ISLAND_HOSTS).flatMap((island) => [
    ...
    ...(['/estimate', '/about', '/events', '/mobile-bar', '/personal-chef', '/vacation-chef', '/legal', '/journal', '/blog', '/locations', '/areas', '/contact', '/trust', '/services', '/help', '/fine-dining', '/staffing', '/corporate', '/gatherings', '/islands', '/sitemap', ...SUPPORT_PATHS] as const).map((path) => ({
