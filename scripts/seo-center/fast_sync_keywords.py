...
DATABASE_URL = "postgresql://neondb_owner:npg_r9qH8ntGCiep@ep-proud-brook-awiy9e7s-pooler.c-12.us-east-1.aws.neon.tech/neondb?sslmode=require"
...
print("Fetching existing pages and keywords into memory...")
cur.execute("SELECT page_id, site_id, slug FROM seo_pages;")
pages_map = {(r["site_id"], r["slug"]): r["page_id"] for r in cur.fetchall()}

cur.execute("SELECT keyword_id, keyword FROM seo_keywords;")
keywords_map = {r["keyword"]: r["keyword_id"] for r in cur.fetchall()}

print(f"Found {len(pages_map)} pages, {len(keywords_map)} keywords.")

# 1. Insert missing keywords
missing_kws = [m for m in MAPPINGS if m[2] not in keywords_map]
print(f"Inserting {len(missing_kws)} new keywords...")
for site_id, slug, kw, intent, vol, cpc, kd in missing_kws:
    cur.execute("""
    ...
    keywords_map[kw] = new_id

# 2. Update pages & ownership
print("Updating page keyword assignments...")
updated = 0
for site_id, slug, kw, intent, vol, cpc, kd in MAPPINGS:
    pid = pages_map.get((site_id, slug))
...
conn.commit()
print(f"Successfully locked and committed {updated} primary keywords in NEO!")
cur.close()
conn.close()
