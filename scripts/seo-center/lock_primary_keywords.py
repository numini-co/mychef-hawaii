#!/usr/bin/env python3
"""
lock_primary_keywords.py
Connects and updates primary keywords in single transaction with direct logging.
"""
import psycopg2
...
def main():
    ...
    for site_id, slug, kw, intent, vol, cpc, kd in MAPPINGS:
        ...
        # 2. Find page
        cur.execute("SELECT page_id FROM seo_pages WHERE site_id = %s AND slug = %s;", (site_id, slug))
        p = cur.fetchone()
        if not p:
            continue
        pid = p[0]
        ...
        # 3. Update page
        cur.execute("UPDATE seo_pages SET primary_keyword_id = %s, primary_intent = %s WHERE page_id = %s;", (kid, intent, pid))
        ...
        """, (kid, pid, site_id))
        count += 1
        print(f"[{count}/{len(MAPPINGS)}] Locked '{kw}' -> [{site_id}] /{slug}")

    conn.commit()
    print(f"\nAll {count} keywords successfully locked in NEO PostgreSQL!")
    cur.close()
    conn.close()
