...
## Wave 148 — In-Villa Services silo: `/in-villa-services` hub + 8 children (this branch)

Blueprint package extracted this run (`/tmp/master_chef/` — `VILLA-OPERATIONS-PAGE-COPY.md`, `pages/00..08`, `strategy/villa-operations-strategy.md`, keyword/SERP/competitor research). The package defines a guest-facing, stay-based **In-Villa Services** silo — the commercially valuable category between a one-night private chef and full owner-side staffing: *"you book the villa; myCHEF runs the food, drinks and service staff for your whole stay."* Its Bali copy (URLs `/in-villa-service/*`, IDR rates) is adapted here to Hawaiʻi (four islands, USD from the canonical rate card).

- **Additive only — zero existing URLs changed.** New apex hub `/in-villa-services` (plural, non-colliding — the owner-side `/staffing`, `/staffing/bartenders`, `/staffing/servers`, `/staffing/butlers` are untouched and kept deliberately separate) plus 8 children: `villa-team` (flagship), `weekly-private-chef`, `butlers`, `waiters`, `bartenders`, `barista`, `villa-provisioning`, `breakfast-service`. All thin `page.tsx` → `InVillaServicesView` → `data/inVillaServices.ts` (a NEW registry the audit does not read, so it cannot fight the uniqueness counts).
- **Uniqueness / audit:** NO photo hero — the `/estimate` paper header (kicker + `LineReveal` H1 + lede), so the audit's per-page-type unique-hero rule is never touched and no existing hero file is reused. Each of the 9 pages has a distinct title/description in `data/pageMeta.ts`. `heroPhotoKey` is reserved on every page so a generated hero can drop in later without a data reshape.
- **Honesty:** dual-model groceries (signature dinner inside the per-guest band; stay chef / weekly / multi-day = chef fee + groceries at cost with receipts). 20% service + Hawaiʻi GET up to `4.712%` itemized; written quote = total; 50% deposit; gratuity voluntary. Every published number derives from `data/rateCard.ts` (Stay Chef day rate, server/sous hourly, packaged bar, vacation-chef per-person/day). Barista and provisioning carry **no** published price (no Hawaiʻi benchmark) and are quoted in writing. Zero reviews/ratings/`AggregateRating`; no fabricated booking/guest counts (the Bali "500+ bookings / 12,000+ guests / HACCP / 2-hour guarantee" trust bar is dropped and replaced with published-policy chips). Kauaʻi and Hawaiʻi Island flagged inquiry-stage per the rate card.
