  ...
  return (
    ...
    >
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="inline-flex min-h-8 items-center gap-1.5 py-1 text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--nav-fg)] hover:opacity-80"
      >
        <span>{label}</span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full z-50 mt-1.5 min-w-[14rem] border border-line bg-paper py-1.5 text-ink shadow-[0_12px_28px_rgba(22,20,18,0.10)]"
          >
            <p className="px-3.5 pb-1.5 pt-2 text-[11px] uppercase tracking-[0.14em] text-mute">
            ...
            <HostLink
              ...
              className="flex min-h-9 items-baseline justify-between gap-3 px-3.5 py-2 text-left text-sm hover:bg-sand"
            >
              <span>All Hawaiʻi</span>
              {!islandId ? (
                <span className="text-[11px] uppercase tracking-[0.12em] text-mute">Here</span>
              ) : null}
            </HostLink>
            {islandOrder.map((id) => (
              <HostLink
