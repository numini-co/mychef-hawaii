...
import { JOURNAL_ARTICLE_SLUGS, getJournalArticle } from '@/data/journalArticles';
...
export function generateStaticParams() {
  return ISLAND_HOSTS.flatMap((island) => JOURNAL_ARTICLE_SLUGS.map((slug) => ({ island, slug })));
}
...
export async function generateMetadata({
  ...
  return islandMetadata(island as IslandId, `/journal/${slug}`);
}
...
export default async function JournalArticlePage({
  ...
  const page = getJournalArticle(islandId, slug);
  if (!page) notFound();
